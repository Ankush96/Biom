...
template <typename T, typename Int> class Tree_Seq {
   ...
   public:
      ...
      void inline create_tree(const int N_variables, int* own_variables,
            int* N_own_variables, T* lambda, Int* groups_ir, Int* groups_jc,
...
template <typename T, typename Int>
void inline Tree_Seq<T,Int>::create_tree(const int N_variables, int* own_variables,
      int* N_own_variables, T* lambda, Int* groups_ir,Int* groups_jc,
...
template <typename T, typename Int>
int inline Tree_Seq<T,Int>::perform_order(const int current_node, const int pointer) {
...
template <typename T, typename Int>
int inline Tree_Seq<T,Int>::perform_dfs(const int current_node, const int pointer) {
   int cur_pointer=pointer;
...
// could be faster
template <typename T, typename Int>
T inline Tree_Seq<T,Int>::val_norm(const T* pr_alpha, const int current_node, const bool l1) {
   T tmp=0;
   return this->val_norm2(pr_alpha,current_node,tmp,l1);
  /* T val = 0;
   if (l1) {
      for (int i = 0; i<_size_variables[current_node]; ++i)
         val=MAX(abs<T>(pr_alpha[i+_pr_variables[current_node]]),val);
   } else {
      val = cblas_nrm2<T>(_size_variables[current_node],pr_alpha+_pr_variables[current_node],1);
   }
   T sum=_lambda[current_node]*val;
   for (int i = _groups_jc[current_node];  i<_groups_jc[current_node+1]; ++i) 
      sum+=this->val_norm(pr_alpha,_groups_ir[i],l1);
   return sum;*/
};

// fast version
template <typename T, typename Int>
T inline Tree_Seq<T,Int>::val_norm2(const T* pr_alpha, const int current_node, T& tmp, const bool l1) {
   T sum=0;
   for (Int i = _groups_jc[current_node];  i<_groups_jc[current_node+1]; ++i) {
      T tmp2=0;
...
template <typename T, typename Int>
T inline Tree_Seq<T,Int>::val_zero2(const T* pr_alpha, const int current_node, bool& tmp) {
   T sum=0;
   for (Int i = _groups_jc[current_node];  i<_groups_jc[current_node+1]; ++i) {
      bool tmp2=false;
...
template <typename T, typename Int>
T inline Tree_Seq<T,Int>::val_zero(const T* pr_alpha, const int current_node) {
   bool tmp = false;
   return this->val_zero2(pr_alpha,current_node,tmp);
};

template <typename T, typename Int>
void inline Tree_Seq<T,Int>::sub_grad(const Vector<T>& input, Vector<T>& output, const bool linf) {
   output.setZeros();
   if (linf) {
      for (int i = 0; i<_N_groups; ++i) {
         ...
         for (int j = 0; j<_size_variables[i];++j) {
            if (abs<T>(max-pr[j]) < 1e-10) ++num_max;
         }
         T add=T(1.0)/num_max;
         for (int j = 0; j<_size_variables[i];++j) {
            if (abs<T>(max-pr[j]) < 1e-10) {
...
template <typename T, typename Int>
void inline Tree_Seq<T,Int>::proj_weighted_linf(Vector<T>& input, const Vector<T>& weights, 
      const T fact) {
   Vector<T> weig;
   weig.copy(weights);
   weig.inv();
...
template <typename T, typename Int>
void inline Tree_Seq<T,Int>::proj(Vector<T>& input, const bool l1,
      const T fact) {
   if (l1) {
...
template <typename T, typename Int>
void inline Tree_Seq<T,Int>::proj_zero(Vector<T>& input, const T fact) {
      cblas_copy<T>(input.n(),input.rawX(),1,_variables,1);
      for (int i = 0; i<_N_groups; ++i) {
         const int node=_order[i];
         _work[node]=0;
...
template <typename T, typename Int>
T inline Tree_Seq<T,Int>::dual_norm_inf(const Vector<T>& input) {
   tglobal1.reset();
   tglobal2.reset();
   tglobal3.reset();
   for (int i = 0; i<_N_groups; ++i) {
...
template <typename T> class MaxFlow {
   ...
   public:
      ...
      T inline norm(const T* variables, T* work, const T* weights, const int Ng);
...
template <typename T>
T inline MaxFlow<T>::norm(const T* variables, T* work, const T* weights, const int Ng) {
   list_int tmp;
   for (int i = 0; i<_N; ++i) {
      _seen[i]=false;
      work[i]=0;
   ...
   while (!tmp.empty()) {
      ...
      if (_seen[node]) {
         ...
         if (node >= Ng && node != _s && node != _t) {
            work[node]=abs(variables[node-Ng]);
            _seen[node]=true;
            tmp.pop_front();
         } else {
            const int ind = _pr_node[node];
            ...
            if (all_child) {
               ...
               for (int i = 0; i<_num_edges[node]; ++i) {
                  ...
                  if (child != _s && child != _t && capacity[i] > 0) {
                     max_var = MAX(max_var,work[child]);
                  }
               }
               work[node]=max_var;
               _seen[node]=true;
...
template <typename T, typename Int = int> class Graph {
   public:
      Graph();
      ~Graph();

      void inline create_graph(const int Nv, const int Ng,
            T* weights, Int* var_ir, Int* var_jc);
      void inline create_graph(const int Nv, const int Ng,
            T* weights, Int* gv_ir, Int* gv_jc, Int* gg_ir, Int* gg_jc);
      ...
      T inline norm(const T* variables, T* work, const T* weights) { return _maxflow->norm(variables,work,weights,_Ng); };
...
template <typename T, typename Int>
void inline Graph<T,Int>::create_graph(const int Nv, const int Ng,
      T* weights, Int* var_ir, Int* var_jc) {
   _Nv=Nv;
   _Ng=Ng;
   _weights=new T[_Ng];
...
template <typename T, typename Int>
T inline Graph<T,Int>::dual_norm_inf(const Vector<T>& input,
...
template <typename T, typename Int>
void inline Graph<T,Int>::proximal_operator(const T* variables_in, T* variables_out,bool clever, const T* weights) {
...
template <typename T, typename Int> struct GraphStruct { 
   Int* gv_ir;
   Int* gv_jc;
   Int* gg_ir;
   Int* gg_jc;
...
template <typename T, typename Int> struct TreeStruct { 
   int* own_variables;
   int* N_own_variables;
   T* weights;
   Int* groups_ir;
